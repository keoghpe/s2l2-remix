...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create s2l2-remix-5e00
  fly apps create s2l2-remix-5e00-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app s2l2-remix-5e00
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app s2l2-remix-5e00-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 's2l2-remix-5e00-staging' does not match app name in config file 's2l2-remix-5e00'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name s2l2-remix-5e00-db
  fly postgres attach --app s2l2-remix-5e00 s2l2-remix-5e00-db

  fly postgres create --name s2l2-remix-5e00-staging-db
  fly postgres attach --app s2l2-remix-5e00-staging s2l2-remix-5e00-staging-db
  ```
